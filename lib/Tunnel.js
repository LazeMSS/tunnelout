...
module.exports = class Tunnel extends EventEmitter {
    ...
    _getInfo(body) {
        const { id, ip, port, url, cached_url, max_conn_count, dashboard } =
            body;
            ...
            axios
                ...
                .then((res) => {
                    ...
                    if (res.status !== 200) {
                        const err = new Error(
                            (body && body.message) ||
                                'tunnelOut server returned an error, please try again!'
                        );
                        return cb(err);
                    }
                    if (
                        !('server' in res.headers) ||
                        res.headers['server'].indexOf(appName) == -1
                    ) {
                        debug('invalid server', res.headers);
                        console.error(
                            'tunnelOut server did not respond with proper server headers: %s',
                            appName
                        );
                        process.exit(1);
                        return false;
                    ...
                    if (
                        ...
                        console.error(
                            'Invalid tunnelOut server response! Missing JSON headers'
                        );
                        process.exit(1);
                        return false;
                    }
                    if (
                        ...
                        console.error(
                            'Invalid tunnelOut server response! Invalid JSON'
                        );
                        process.exit(1);
                        return false;
                    }
                ...
                .catch((err) => {
                    if (opt.retries > 0) {
                        failCount++;
                        debug(
                            `tunnel server offline: ${err.message}, retry 1s, failCount: ${failCount} / ${opt.retries}`
                        );
                        ...
                        debug(
                            `tunnel server offline: ${err.message}, retry 1s`
                        );
                    }
    ...
    _establish(info) {
        ...
        this.tunnelCluster.on('throttle', () => {
            ...
            if (this.openTunnels < max_conn || this.openTunnels <= 2) {
                debug(
                    'Creating next tunnelCluster %s / %s',
                    this.openTunnels,
                    max_conn
                );
                this.tunnelCluster.open();
