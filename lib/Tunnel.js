        ...
        (function getUrl() {
            axios
                ...
                .then(res => {
                    ...
                    if (res.status !== 200) {
                        const err = new Error(
                            (body && body.message) || 'tunnelOut server returned an error, please try again!'
                        );
                        return cb(err);
                    }
                    /*if (!('server' in res.headers) || res.headers['server'] != 'tunnelout'){
                        debug('invalid server',res.headers);
                        console.error('tunnelOut server did not respond with proper server headers');
                        process.exit(1);
                        return false;
                    }
                    // Json?
