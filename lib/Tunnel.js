        ...
        (function getUrl() {
            axios
                ...
                .then(res => {
                    ...
                    if (!("x-server-type" in res.headers) || res.headers["x-server-type"] != "localtunnel"){
                    ...
                    if (!("content-type" in res.headers) || res.headers["content-type"].indexOf("application/json") != 0){
                        debug("invalid server - not json");
                        console.error("Invalid server response! Not JSON returned");
                        process.exit(1);
                        return false;
                    }
                    if (res.status !== 200) {
